...
-   **自定义数据源**：支持通过配置文件接入任何 REST API、RSS Feed 或 HTML 网站，无限扩展您的信息来源。
-   **智能内容过滤**：除了基础的关键词匹配，还支持正则表达式、模糊匹配和自动简繁/全半角转换，实现专家级的内容筛选。

**➡️ [点击查看高级功能详细指南](docs/advanced_features.md)**
